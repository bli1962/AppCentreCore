...
namespace mhcb.Syd.Api.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        //[AllowAnonymous]
        [Authorize]
        [Route("GetGuidePasswordByUserId/{userId}")]
        [HttpGet]
        public ActionResult<UserView> LoadGuidePasswordByUserId(string userId)
        {
            try
            {
                if (userId == "")
                ...
                var entry = _repository.LoadGuidePasswordByUserId(userId);
                if (entry != null)
                {
                    // HTTP/1.1 200 OK 
                    return Ok(entry);
        ...
        [Route("")]
        [HttpPut]
        public ActionResult<int> Put([FromBody] InputGuideUser userAttr)
        {
            try
            {
